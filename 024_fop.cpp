...
for(int i = 0; i < datavector.size(); ++i)
{
    ...
    if (linesplitvector.size() == 3)
    {
        ...
        {
            cout << "Expected age expressed as an integer in line: " << i << endl;
        }
    }
    else if (linesplitvector.size() > 3)
    {
        cout << "Too many words in line number: " << i << endl;
    }
    else if (linesplitvector.size() < 3)
    {
        cout << "Not enough words in line number: " << i << endl;
    }
}
...
{
    cout << "line nr: " << i << " - " << peoplevector[i].name << " " << peoplevector[i].surname << " " << peoplevector[i].age << endl;
}
bool active1;
active1 = true;
bool active2;
        ...
        while (active2)
        {
            ...
            if (x=="CHANGE")
            {
                ...
                while (line_flag)
                {
                    ...
                    int line_number_int;
                    cin >> line_number_string;
                    try
                    {
                        line_number_int = stoi(line_number_string);
                        if (line_number_int<peoplevector.size())
                        {
                            ...
                            while (age_flag)
                            {
                                ...
                                try
                                {
                                    ...
                                    cout << "Ok, the person: " << guy.name << " " << guy.surname << " " << guy.age << " has been added to the line " << line_number_int << endl;
                                }
                                catch(invalid_argument)
                                {
